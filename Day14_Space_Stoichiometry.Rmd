...
calc_needed <- function(label, need) {
  ...
  if (need <= min_make){
    ...
    calc <- {need %/% min_make} + 1L
  }
  calc
}

groupem <- function(.df) { count(.df, label, wt = val) }
...
required <- required %>% add_row(label = "FUEL", val = 1L)

required[!(required$label %in% ore_labels), ] %>% 
  inner_join(recipes, by = c("label" = "out_label")) %>% 
  rowwise() %>% 
  mutate(have = have(in_label), 
         make = calc_needed(label, (val - have)), 
         add = val * in_val, 
         leftover = make - add)
